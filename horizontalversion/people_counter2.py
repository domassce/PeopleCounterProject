...
def log_data(move_in, in_time, move_out, out_time):
	...
	with open('utils/data/logs/counting_data.csv', 'a', newline = '') as myfile:  # Changed 'w' to 'a'
	...
	while True:
		...
		for (objectID, centroid) in objects.items():
			...
			# to determine direction
				...
				to.last_seen = datetime.datetime.now()
				...
				if not to.counted:
					 # calculate the time difference since the object was first seen
					time_diff = (to.last_seen - to.first_seen).total_seconds()

					# if the direction is negative (indicating the object
					# is moving up) AND the centroid is above the center
					# line, count the object
					if direction < 0 and centroid[1] < H // 2 and time_diff > 2:
						totalUp += 1
					...
					elif direction > 0 and centroid[1] > H // 2 and time_diff > 2:
						totalDown += 1
						date_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
