	...
	while True:
		...
		# Check for objects that disappeared
			...
			for objectID in disappeared_objects:
				...
				# Compare first and last X position to determine movement direction
				if to.first_x - to.last_x > 20:  # Moved left
					totalUp += 1
					move_out.append(totalUp)
					move_out.append(datetime.datetime.now().strftime("%Y-%m-%d %H:%M"))

				elif to.last_x - to.first_x > 20:  # Moved right
					totalDown += 1
					move_in.append(totalDown)
					move_in.append(datetime.datetime.now().strftime("%Y-%m-%d %H:%M"))

				del trackableObjects[objectID]  # Remove tracked object to avoid re-counting
